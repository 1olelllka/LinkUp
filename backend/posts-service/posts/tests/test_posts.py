...
class TestPostsAPI(TestCase):
    ...
    def test_create_post_for_specific_user_returns_Http400(self):
        response = self.client.post("/posts/users/1", data={"user_id": 1, "desc": "something"})
        self.assertEqual(response.status_code, 400)
    
    def test_create_post_for_specific_user_returns_Http200(self):
    ...
    def test_delete_specific_post_returns_Http204(self):
        response = self.client.delete("/posts/1234")
        self.assertEqual(response.status_code, 204)

    def test_get_posts_by_users_returns_list_of_posts(self):
        response = self.client.get("/posts/", {"user_ids": "{1,2,3}"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 2)
    
    def test_get_posts_by_users_returns_empty_list(self):
