...
class TestPostsAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.user1_post = Post.objects.create(user_id=self.user1_id, desc="User 1 - Post 1", image="url")
        self.user2_post = Post.objects.create(user_id=self.user2_id, desc="User 2 - Post 1", image="url")
        self.mongo_profile = {"username": "some_username", "name": "test_name", "photo": ""}

    def test_get_posts_by_specific_user(self, *args):
        response = self.client.get(f"/posts/users/{self.user1_id}")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()['results']), 1)

    @patch("posts.views.linkup_collection.find_one")
    @patch("jwt.decode")
    def test_create_post_for_specific_user_returns_Http400(self, mock_jwt, mock_find_one, *args):
        mock_jwt.return_value = {'sub': self.user1_id}
        mock_find_one.return_value = self.mongo_profile
        response = self.client.post(
            f"/posts/users/{self.user1_id}",
            data={"user_id": self.user1_id, "desc": "something"},
            headers={"Authorization": "Bearer jwt_token"}
        )
        self.assertEqual(response.status_code, 400)

    @patch("jwt.decode")
    def test_create_post_for_specific_user_returns_Http403(self, mock_jwt, *args):
        mock_jwt.return_value = {'sub': str(uuid.uuid4())}  # not the owner
        response = self.client.post(
            f"/posts/users/{self.user1_id}",
            data={"user_id": self.user1_id, "desc": "something"},
    ...
    @patch("posts.views.linkup_collection.find_one")
    @patch("jwt.decode")
    def test_create_post_for_specific_user_returns_Http201(self, mock_jwt, mock_find_one, *args):
        mock_jwt.return_value = {'sub': self.user1_id}
        mock_find_one.return_value = self.mongo_profile
        response = self.client.post(
            f"/posts/users/{self.user1_id}",
            {"desc": "something", "image": "url"},
            headers={"Authorization": "Bearer jwt_token"}
        )
    ...
    @patch("posts.views.linkup_collection.find_one")
    @patch("jwt.decode")
    def test_create_post_for_specific_user_returns_Http404(self, mock_jwt, mock_find_one, *args):
        mock_find_one.return_value = None
        profile_id = uuid.uuid4()
        mock_jwt.return_value = {"sub": str(profile_id)}
        response = self.client.post(
            f"/posts/users/{profile_id}",
            {"user_id": str(profile_id), "desc": "something", "image": "url"},
            headers={"Authorization": "Bearer jwt_token"}
        )
    ...
    def test_patch_specific_post_returns_Http404(self, mock_jwt, *args):
        ...
        response = self.client.patch(
            "/posts/123",
            {"user_id": self.user1_id, "desc": "desc", "image": "image"},
            headers={"Authorization": "Bearer jwt_token"}
        )
        self.assertEqual(response.status_code, 404)

    def test_patch_specific_post_returns_Http403(self, *args):
        response = self.client.patch(
            "/posts/123",
            {"user_id": self.user1_id, "desc": "desc", "image": "image"}
        )
    ...
    def test_patch_specific_post_returns_Http400(self, mock_jwt, *args):
        ...
        response = self.client.patch(
            f"/posts/{self.user1_post.pk}",
            {"user_id": self.user1_post.user_id},
            headers={"Authorization": "Bearer jwt_token"}
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_patch_specific_post_returns_Http200(self, mock_cache_set, mock_jwt, *args):
        ...
        response = self.client.patch(
            f"/posts/{self.user1_post.pk}",
            {"user_id": self.user1_post.user_id, "desc": "UPDATED", "image": "UPDATED"},
            headers={"Authorization": "Bearer jwt_token"}
        )
        self.assertEqual(response.status_code, 200)
